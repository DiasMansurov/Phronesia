...
3. Search `SPY`; select it. The selected asset card should show a cached/API stock price from the server-side `/stocks/quotes` pipeline, or "No saved price yet. Select the asset to fetch the latest price."
...
13. Confirm `/api/investment/debug-price?symbol=SPY` uses `/stocks/quotes` and never returns the API token.
